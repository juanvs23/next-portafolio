...
const Form = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      nombre.current.value.length < 3 ||
      email.current.value.length <= 0 ||
      phone.current.value.length == "" ||
      !regex.test(String(email.current.value).toLowerCase())
    ) {
  ...
  return (
    <Fomulario>
      <form id="contact-form" onSubmit={handleSubmit} method="post">
        ...
        <button type="submit" id="send" className="btn btn-primary btn-large">
          Enviar
        </button>
      </form>
      <Modal
      ...
      >
        <Modal.Body>
          ...
          <p className="p-2 text-center text-white">
            Pronto me contactare con usted. Gracias por mostrar interes en mi
            trabajo, espero poder ayudarlo. Mientras lo invito a revisar mi blog
            y también revisar mis proyectos públicos es github
          </p>
        </Modal.Body>
        <Modal.Footer>
          <button className="m-auto btn btn-info" onClick={closeModal}>
